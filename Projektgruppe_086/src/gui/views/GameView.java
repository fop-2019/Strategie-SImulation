...
public class GameView extends View implements GameInterface {
    ...
    public void onNextTurn(Player currentPlayer, int troopsGot, boolean human) {
        ...
        if(game.getRound() == 1)
        	...
        	//changed to exclude this message if deathmatch mode is on : Luca
        	if (game.getGoal().gameModeID != 3) /*changed*/ {
    ...
    public void onNewRound(int round) {
    	if (round == 2 && game.getGoal().gameModeID == 1) {
    		JOptionPane.showMessageDialog(null, ("Fahnenburg ist: " + game.getFlagCastle().getName()), "Erobere die Fahnenburg und halte Sie!", JOptionPane.PLAIN_MESSAGE);
    	}
