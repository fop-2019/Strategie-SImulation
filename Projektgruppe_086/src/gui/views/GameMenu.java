...
public class GameMenu extends View {
	...
	protected void onInit() {
		...
		goal.addItemListener(itemEvent2 -> {
			...
			if (index == 1 || index == 2 || index == 3) {
				...
				playerTypes.add(c.getSimpleName());
				for (int i = 0; i<GameConstants.MAX_PLAYERS; i++) {
					((JComboBox) playerConfig[i][3]).setSelectedIndex(0);
				}
			} else {
				playerTypes.removeAll(playerTypes);
				for (Class<?> c : GameConstants.PLAYER_TYPES) {
	...
	private void updateCombo(JComboBox box, Vector<String> types) {
		box.removeAllItems();
		for (String t : types) {
			box.addItem(t);
		}
	...
	public void actionPerformed(ActionEvent actionEvent) {
		if (actionEvent.getSource() == btnBack)
			...
			try {
				...
				game.setGoal(goal);
				getWindow().setView(new JokerMenu(getWindow(), game, this));
